...
def run_model_LastFM(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    features_issue = np.load(prefix + '/features-gptllama/drill-features-Gptllama.npy')
    features_reduced = pca.fit_transform(features_issue)
    features_0 = torch.tensor(features_reduced, dtype=torch.float32)
    # features_0 = np.load(prefix + '/features_issue.npy')
    # features_0 = np.random.rand(num_issues, 10)
    ...
    train_pos_user_artist = train_val_test_pos_user_artist['train'] #[64984,2]
    val_pos_user_artist = train_val_test_pos_user_artist['val']# [9283,2]
    test_pos_user_artist = train_val_test_pos_user_artist['test'] #[18567,2]
