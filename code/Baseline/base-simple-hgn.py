...
class SimpleHGNConv(nn.Module):
    """Simple Heterogeneous Graph Neural Network Layer"""
    ...
    def __init__(self, in_channels, out_channels, node_types, edge_types):
        ...
        # Self-transformation for each node type
        self.self_transforms = nn.ModuleDict()
        for node_type in node_types:
            self.self_transforms[node_type] = nn.Linear(in_channels, out_channels)
        ...
        for src_type, edge_type, dst_type in edge_types:
            ...
            self.edge_transforms[edge_key] = nn.Linear(in_channels, out_channels)
    ...
    def forward(self, x_dict, edge_index_dict):
        # Self-transformation for all node types
        h_self = {}
        for node_type, x in x_dict.items():
            h_self[node_type] = self.self_transforms[node_type](x)
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            edge_key = f"{src_type}__{relation}__{dst_type}"

            if edge_key in self.edge_transforms and src_type in x_dict:
                # Transform source features
                src_features = x_dict[src_type]
                transformed_features = self.edge_transforms[edge_key](src_features)
                ...
                if edge_index.size(1) > 0:
                    ...
                    aggregated = torch.zeros(
                        h_self[dst_type].size(0),
                        self.out_channels,
                        device=transformed_features.device,
                        dtype=transformed_features.dtype
                    )

                    # Sum messages for each destination node
                    aggregated.index_add_(0, col, transformed_features[row])
                    ...
                    degree.index_add_(0, col, torch.ones(col.size(0), device=transformed_features.device,
                                                         dtype=transformed_features.dtype))
                    degree = degree.clamp(min=1).unsqueeze(1)

                    # Normalize by degree (mean aggregation)
                    normalized_messages = aggregated / degree
                    messages[dst_type].append(normalized_messages)

        # Combine self-features with aggregated messages
        output = {}
        for node_type in self.node_types:
            if messages[node_type]:
                # Average all message types for this node type
                aggregated_messages = torch.stack(messages[node_type], dim=0).mean(dim=0)
                # Combine with self-transformation
                output[node_type] = self.activation(h_self[node_type] + aggregated_messages)
            else:
                # No incoming messages, use only self-transformation
                output[node_type] = self.activation(h_self[node_type])

            # Apply dropout
            output[node_type] = self.dropout(output[node_type])

        return output


class SimpleHGNLinkPredictor(nn.Module):
    """Simple-HGN model for heterogeneous link prediction"""

    def __init__(self, hidden_channels, out_channels, num_layers, node_types, edge_types, dropout=0.2):
        super().__init__()
        ...
        # Simple-HGN layers (using regular nn.Linear since we know dimensions)
        self.convs = nn.ModuleList()
        for i in range(num_layers):
            conv = SimpleHGNConv(hidden_channels, hidden_channels, node_types, edge_types)
            self.convs.append(conv)

        # Output projection (using regular nn.Linear)
        self.output_projection = nn.Linear(hidden_channels, out_channels)

        # Link prediction decoder (using regular nn.Linear)
...
class JiraDataLoader:
    ...
    def load_data(self):
        ...
        print("=" * 60)
        print("LOADING JIRA DATASET FOR SIMPLE-HGN")
        print("=" * 60)
...
def get_next_run_id(save_path='simple_hgn_results.csv'):
...
def save_results_to_csv(test_metrics, runtime, config, run_id=None, seed=None, save_path='simple_hgn_results.csv'):
...
def run_simple_hgn_experiment(seed_type='fixed', fixed_seed=42, data_path='../AIL/data/'):
    """
    Main experimental pipeline for Simple-HGN
    ...
    print("=" * 80)
    print("SIMPLE-HGN HETEROGENEOUS LINK PREDICTION EXPERIMENT")
    print("=" * 80)
    ...
    print("=" * 50)
    print(f"Architecture: Simple-HGN")
    ...
    print(f"Edge types: {len(edge_types)} (No metapaths required!)")
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        if epoch % 10 == 0:
            ...
            if val_metrics['roc_auc'] > best_val_auc + config['min_improvement']:
                ...
                best_epoch = epoch
                patience_counter = 0
                torch.save(model.state_dict(), 'best_simple_hgn_model.pt')
    ...
    # Load best model
    model.load_state_dict(torch.load('best_simple_hgn_model.pt'))
    ...
    save_results_to_csv(
        ...
        config,
        seed=seed,
        save_path='simple_hgn_results.csv'
    )

    print(f"\n" + "=" * 80)
    print("SIMPLE-HGN EXPERIMENT COMPLETED!")
    print("=" * 80)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Simple-HGN Link Prediction Experiment')
...
if __name__ == "__main__":
    ...
    if args.runs == 1:
        # Single run
        results = run_simple_hgn_experiment(
