...
#解析邻接列表，生成图结构数据和元路径索引
def parse_adjlist(adjlist, edge_metapath_indices, samples=None):
    edges = []
    nodes = set()
    result_indices = []  # 采样后的元路径索引数组
    for row, indices in zip(adjlist, edge_metapath_indices):
        row_parsed = list(map(int, row.split(' ')))
        nodes.add(row_parsed[0])

        if len(row_parsed) > 1:
            # sampling neighbors如果有邻居
            if samples is None:
                neighbors = row_parsed[1:]
                result_indices.append(indices)
            else:
                # undersampling frequent neighbors采样邻居
...
##出现问题：邻接列表 adjlist 的某行的邻居数量与对应的元路径索引 edge_metapath_indices 的长度不一致，导致采样时生成的索引越界
        else:
            # 无邻居
            neighbors = []
            result_indices.append(indices)
